...
class Triangle:
    """Klasa Triangle z zestawu 6, rozbudowana o obsługę błędów, metodę make4
    oraz metodę from_points. Ponadto klasa Triangle posiada teraz atrybuty
    wirtualne opisujące prostokąt ograniczający dany trójkąt."""
    ...
    def from_points(cls, points):
        if len(points) != 3:
            raise ValueError("You should pass exactly three points!")

        if not all(isinstance(p, Point) for p in points):
            raise TypeError("All elements must be instances of Point!")
    ...
    # Atrybuty wirtualne opisujące prostokąt ograniczający dany trójkąt,
    # zwracają liczbę (współrzędną tylko do odczytu):
    @property
    def top(self): return max(self.pt1.y, self.pt2.y, self.pt3.y)

    @property
    def left(self): return min(self.pt1.x, self.pt2.x, self.pt3.x)

    @property
    def bottom(self): return min(self.pt1.y, self.pt2.y, self.pt3.y)

    @property
    def right(self): return max(self.pt1.x, self.pt2.x, self.pt3.x)
    ...
    # Atrybuty wirtualne opisujące prostokąt ograniczający dany trójkąt,
    # zwracające Point (tylko do odczytu):
    @property
